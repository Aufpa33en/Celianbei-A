...
第三问已经完成模型推导、三路代理审查、六模型smoke、40块完整电池嵌套LOBO全量验证、模型冻结、9块测试电池第151—200循环预测、保守逐循环点区间、T80情景敏感性、短期预测误差向T80的传播诊断和论文图。Q3不再停留于smoke阶段。

## 权威目录

- `01_smoke_test/`：六个候选模型的工程排错与估时历史，不作为最终选模结果。
- `02_full_validation/`：40块完整电池的外层LOBO、L=150专用嵌套选模、策略内配对bootstrap、消融、调参与完整性审计。
- `03_final_predictions/`：冻结`P1_linear`后对9块测试电池的151—200循环预测、经验97.5%残差分位点区间、T80情景敏感性、整段残差轨迹传播诊断和论文图。
...
- 最终任务拥有150个观测循环，因此按L=150固定候选外层LOBO冻结`P1_linear`；其策略等权RMSE为0.000617，优于C的0.000669，差异超过2%并列容差。该数值是固定P1候选的误差，不冒充完整选模流程误差。
- L=150专用嵌套选模流程在每个外折只用其余39块电池选族，策略等权RMSE为0.000704；40折中选择P1/C/D分别为25/1/14次。5000次策略内配对bootstrap中，固定候选P1的赢家频率为81.04%。
- 50/100/150综合指标仍由`C_ridge`领先，但只作多长度鲁棒性敏感性；原多长度动态选族流程在L=150的RMSE为0.000701，不能替代L=150专用流程。
- C消融显示策略特征并非稳定增益；事后消融变体不追加进本轮六个预注册候选。
- 预测区间以模型族已冻结为P1为条件，不包含选族不确定性，也不是独立测试覆盖率。
- 49块电池均没有真实80% SOH终点；T80只作情景外推，不能报告为已验证寿命。
- `t80_uncertainty_draws.csv`按整段151—200外层残差轨迹重采样，共9×2000次；其2.5%—97.5%范围是选模后误差传播诊断，不是T80置信区间，也没有覆盖远期机理不确定性。
