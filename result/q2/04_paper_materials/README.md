...
1. **只能诊断，不能确认**：若额外假设固定协议组之间的电池标签可交换，九种策略末段退化率的20000次Monte Carlo标签置换尾部比例为`0.000050`；但组样本数和方差不等，且协议不是随机分派，因此这不是确认性`p`值。SOH200的36组两两精确检验经Holm校正后为0组显著，具体策略对只能作描述性排序。
...
4. **当前样本无法验证**：`J+H`对数速率模型在明确新结构队列中每个留一坐标折仅有5个训练策略，却要拟合含截距的3个系数；误差高度不稳定，其37.4% SOC换向点不进入最终结论。这不证明更大样本下该模型必然无效。
