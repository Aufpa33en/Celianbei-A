...
- `figures/`：两张可直接用于论文的Q2图及其数据来源、统计口径和复现命令。
...
- 参数模型验证：以唯一参数坐标为分组单位留出；使用2000次整块电池bootstrap和删除敏感性分析。720种策略均值标签排列仅保留为不成立交换性假设下的诊断，不作检验。
...
3. **不能确认**：现有非随机策略设计没有有效的参数效应确认性`p`值；删除3.7C–5.9C极端策略后常数模型胜出，故不能声称某个SOC阈值或`C1、Q1、C2`的独立因果效应已经显著。
...
- 策略均值交换性诊断（历史文件名）：`../03_formal_validation/permutation_test_summary.csv`
