...
\end{equation}
建立函数型岭回归主模型：先对每块电池的 SOH 曲线做三次样条岭平滑，再在策略内等权平均电池系数得到策略级曲线，即
\begin{equation}
\mathrm{SOH}_{i,t}=\bm B(x_t)^\top\big(\bm\gamma+\bm\gamma_{p(i)}\big)+\varepsilon_{i,t},
\end{equation}
其中 $\bm\gamma$ 为总体退化系数，$\bm\gamma_{p(i)}$ 为策略 $p(i)$ 的偏离，按每块电池等权而非每条循环记录等权。以样条混合（spline\_mixed，带电池随机截距与随机斜率）与多项式混合（polynomial\_mixed）为候选对照。模型用 40 块完整电池按整块电池留一验证（LOBO）评价，策略级指标以整块电池 bootstrap 估计标准误与 95\% 区间。主模型训练残差的一阶自相关约为 $0.71$，说明平滑后仍存在循环内序列相关，故不确定性统一用整块电池 bootstrap 处理。
...
\subsection{统计显著性}
对策略间末段退化率做全局置换检验：$F=12.50$、置换 $p=5\times10^{-5}$（20000 次置换），表明\textbf{策略标签与末段退化存在全局关联}。在单暴露模型中，高 SOC 区间的倍率暴露（$J_{\mathrm{high},50/60/70}$）均与退化方向一致，其中选定模型 $J_{\mathrm{high},70}$（SOC 高于 70\% 区间的倍率暴露）的标准化系数为 $0.0039$（对相对损失为正、对 SOH200 为负），单暴露未校正 $p=0.019$；但经四个暴露量的选择校正后 $p=0.065$ 未达显著，且删除极端策略后常数模型胜出（图 \ref{fig:q2_stab}）。因此，$J_{\mathrm{high}}$ 与退化的关联方向稳健，但\textbf{不能据此宣称 SOC 阈值或参数的独立因果效应}。2000 次整块电池 bootstrap 进一步显示，$J_{\mathrm{high}}$ 系数 100\% 为正（方向稳健），而 $H$ 相对常数基线的拟合改善为负，支持"高 SOC 区间应力而非总体应力是更一致的退化解释量"这一判断。
...
\subsection{模型选择与预测}
六类候选模型的 $L=150$ 策略等权 LOBO RMSE 与综合评分见表 \ref{tab:q3models}。$C\_ridge$ 按多长度综合评分胜出并冻结；$P1\_linear$（线性外推）在 $L=150$ 单点略优，但综合评分与最差电池 RMSE 均劣于 $C\_ridge$，故最终模型应解释为\textbf{多长度综合稳健最优}而非每个长度下都最优。

\begin{table}[H]\centering
\caption{六类候选模型 $L=150$ 排名}\label{tab:q3models}
\begin{tabular}{clccc}
\toprule
排名 & 模型 & 策略等权 RMSE & 最差电池 RMSE & 选定\\
\midrule
1 & $C\_ridge$（轨迹岭回归） & 0.000660 & 0.004152 & \textbf{是}\\
2 & $P1\_linear$（线性外推） & 0.000617 & 0.004977 & 否\\
3 & $D\_ensemble$（集成） & 0.000704 & 0.006843 & 否\\
4 & $B\_strategy$（策略迁移） & 0.000785 & 0.007377 & 否\\
5 & $A\_power$（幂律） & 0.000893 & 0.006613 & 否\\
6 & $P0\_persistence$（持久化） & 0.002400 & 0.012240 & 否\\
\bottomrule
\end{tabular}
\end{table}

$C\_ridge$ 固定模型族在 $L=150$ 的策略等权 LOBO RMSE 为 $0.000660$，含调参选族的嵌套流程为 $0.000709$；随早期长度增加误差单调下降（图 \ref{fig:q3_len}）。$L=150$ 特征消融显示（表 \ref{tab:q3ab}）：仅用动态轨迹特征的 RMSE $0.000645$ 反而略优于完整模型，说明策略参数特征在该窗口不提供增量精度。
...
点估计 Pareto 前沿含 3 个策略（表 \ref{tab:q4}），bootstrap 显示前沿成员不稳定；$5\_3C\_54PER\_4C$ 的进入频率最高（0.892），$3\_6C$ 与 $4\_8C$ 新结构分别为 0.740 与 0.514。
...
\textbf{末段斜率口径揭示一处反转}：$3\_6C$ 基准的 200 循环累计退化点估计最低（0.000428），但其末段（151--200）每循环衰减率 $0.0000500$ 反而高于 $4\_8C$ 新结构（$0.0000277$）与 $5\_3C$（$0.0000315$），因此在末段斜率前沿上被二者支配。只有 $4\_8C$ 新结构与 $5\_3C$ 在\textbf{SOH200 与末段斜率两个口径下都位于前沿}，未表现出快充导致的末段加速恶化；而 $3\_6C$ 的累计退化均值仅来自 $n=2$ 样本且策略内标准差高于均值本身，不宜对"最低退化"作过强断言。
...
推荐合理性来自四条证据：位于点估计 Pareto 前沿、bootstrap Pareto 频率最高、两种时间口径下前沿不变、SOH200 与末段斜率方向一致。除单点推荐外，本文还给出"给定退化容忍度 $D_{\max}$ 下选择最短时间"的条件决策：当 $D_{\max}=0.0005$ 时 $3\_6C$ 基准被选频率 $73.3\%$（并有 $23.8\%$ 无可行策略）；当 $D_{\max}=0.0017$ 时 $5\_3C$ 被选频率 $51.7\%$。这些容忍度仅为展示决策规则的示例场景，不是工程安全标准。结论严格限定在 9 个已观测策略范围内，不宣称 $C_1,Q_1,C_2$ 的独立因果最优值，不外推到未观测倍率、SOC 区间或新电芯结构\upcite{keil2016charging,schuster2015nonlinear}。
