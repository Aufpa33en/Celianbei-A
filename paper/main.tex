...
各策略第 200 循环 SOH 的 bootstrap 点估计与区间见表 \ref{tab:q1}（部分策略）。典型长寿命策略为 $3\_6C$--$80PER\_3\_6C$（SOH200 $=0.9968$）与 $5C\_67PER\_4C$（$0.9955$）；典型短寿命策略为 $3\_7C\_31PER\_5\_9C$（$0.9659$，最差）、$80PER\_3\_6C$（$0.9806$）与 $4\_8C\_80PER\_4\_8C$（$0.9854$）。

\begin{table}[H]\centering
\caption{各策略 SOH@200 点估计与 bootstrap 区间}\label{tab:q1}
\begin{tabular}{lccc}
\toprule
策略 & SOH200 点估计 & 95\% 区间 & 充电时间/min\\
\midrule
$3\_6C$--$80PER\_3\_6C$ & 0.9968 & [0.9951, 0.9985] & 13.388\\
$5C\_67PER\_4C$ & 0.9955 & [0.9950, 0.9960] & 10.159\\
$5\_3C\_54PER\_4C$ & 0.9954 & [0.9946, 0.9963] & 10.158\\
$4\_8C\_80PER\_4\_8C$ & 0.9854 & [0.9814, 0.9895] & 13.082\\
$80PER\_3\_6C$ & 0.9806 & [0.9676, 0.9935] & 14.280\\
$3\_7C\_31PER\_5\_9C$ & 0.9659 & [0.9603, 0.9714] & 10.852\\
\bottomrule
\end{tabular}
\end{table}

\subsection{两两比较与寿命终点边界}
精确置换检验配合 Holm 校正后，\textbf{36 组 SOH200 两两比较中没有任何策略对显著}——9 种策略每类仅 3--8 块电池，统计功效不足，故排序仅作为描述性结论。此外，49 块电池均未观测到 80\% SOH 终点，局部线性外推的 $L_{80}$ 只能作为未验证的外推代理，不作为寿命标签。
...
\end{equation}
$J$ 描述主要充电区间的总体倍率暴露，$H$ 对高 SOC 区间赋予更高权重。二者相关系数为 $0.87$，$T_0$ 与 $J$ 相关系数为 $-0.984$，三者不能同时进入正式模型。
...
\subsection{统计显著性}
对策略间末段退化率做全局置换检验：$F=12.50$、置换 $p=5\times10^{-5}$（20000 次置换），表明\textbf{策略标签与末段退化存在全局关联}。高 SOC 倍率暴露（$H$）与退化方向一致，但选择校正后 $p=0.065$ 未达显著，且删除极端策略后常数模型胜出（图 \ref{fig:q2_stab}）。
...
\section{问题三模型的建立与求解}
\subsection{特征与候选模型}
从早期循环提取 SOH 轨迹特征，比较六类候选模型：轨迹岭回归（$C\_ridge$）、线性外推、持久化、幂律、策略迁移与机器学习基线等。以 40 块完整电池截断至 $L\in\{50,100,150\}$ 循环做外层留一电池、折内调参、嵌套选族。
