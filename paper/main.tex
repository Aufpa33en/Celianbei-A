...
精确置换检验配合 Holm 校正后，\textbf{36 组 SOH200 两两比较中没有确证显著的策略对}；但另有 20 组未校正 bootstrap 区间不跨零。论文并列报告两者，既不把前者解释为九种策略完全等效，也不把后者当作多重比较后的确证差异——每策略仅 2--7 块完整电池时两两检验功效有限。此外，49 块电池均未观测到 80\% SOH 终点，局部线性外推的 $L_{80}$ 只能作为未验证的外推代理，不作为寿命标签。
...
比较六类候选模型，主模型 $P1\_linear$（线性外推）用前 $L$ 循环拟合线性趋势并外推未来 SOH。\textbf{因部署时固定提供 150 个观测循环，按 $L=150$ 外层留一电池冻结 $P1\_linear$}：其在 $L=150$ 的策略等权 LOBO RMSE 为 $0.000617$（表 \ref{tab:q3models}）。轨迹岭模型 $C\_ridge$ 在多长度综合指标上排名第一，但该指标是将不同 $L$ 的误差加权合成，与最终固定 $L=150$ 的部署场景不同，故保留为\textbf{多长度综合稳健性敏感性}而非部署模型。六模型排名见表 \ref{tab:q3models}，第 2--4 名处于约 2\% 并列容差内，名次不稳定。
...
推荐合理性来自：位于点估计 Pareto 前沿、bootstrap Pareto 频率最高（0.890）、两种时间口径下前沿不变、SOH200 与末段斜率结论方向一致。结论严格限定在 9 个已观测策略范围内，不宣称 $C_1,Q_1,C_2$ 的独立因果最优值，不外推到未观测倍率、SOC 区间或新电芯结构\upcite{keil2016charging,schuster2015nonlinear}。
