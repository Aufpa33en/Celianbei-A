...
from .lifetime_model_outputs import draw_lifetime_family_comparison
...
def write_authoritative_outputs(
    ...
    paper_tables = {
        ...
        "lifetime_family_validation_summary": tables["lifetime_family_validation_summary"],
        "lifetime_family_strategy_t80": tables["lifetime_family_strategy_t80"],
        "lifetime_family_strategy_envelope": tables["lifetime_family_strategy_envelope"],
        "lifetime_family_battery_envelope": tables["lifetime_family_battery_envelope"],
    }
    ...
    draw_lifetime_family_comparison(
        {
            "frozen_candidate_origin_sensitivity": tables["lifetime_family_origin_sensitivity"],
            "strategy_t80_by_family": tables["lifetime_family_strategy_t80"],
            "nested_family_summary": tables["lifetime_family_validation_summary"],
        },
        paper_dir / "fig_q1_lifetime_family_comparison.png",
    )
...
def _draw_final_strategy_curves(tables: dict[str, pd.DataFrame], path: Path) -> None:
    ...
    policies = tables["strategy_lifetime_rank_stability"].sort_values("PointT80Rank")["Policy"]
...
def _draw_final_tradeoff(tables: dict[str, pd.DataFrame], path: Path) -> None:
    ...
    rank = tables["strategy_lifetime_rank_stability"][["Policy", "PointT80Rank"]].copy()
    rank["PrimaryGroup"] = np.select(
        [rank["PointT80Rank"] <= 3, rank["PointT80Rank"] >= 7],
        ["typical_long", "typical_short"],
        default="middle",
    )
    lifetime = tables["strategy_lifetime_summary"][[
        "Policy", "MedianEstimatedT80", "BootstrapCI95Low", "BootstrapCI95High"
    ]].copy()
    charge = scalars.loc[scalars["Metric"] == "MeanChargeTime", ["Policy", "Estimate"]].rename(
        columns={"Estimate": "MeanChargeTime"}
    )
    frame = lifetime.merge(charge, on="Policy").merge(rank, on="Policy")
    ...
    for _, row in frame.iterrows():
        ax.errorbar(
            row["MeanChargeTime"], row["MedianEstimatedT80"],
            yerr=[
                [row["MedianEstimatedT80"] - row["BootstrapCI95Low"]],
                [row["BootstrapCI95High"] - row["MedianEstimatedT80"]],
            ],
            fmt="o", color=colors[row["PrimaryGroup"]], capsize=3, markersize=7,
        )
        rank_value = int(row["PointT80Rank"])
        ax.annotate(f"R{rank_value}", (row["MeanChargeTime"], row["MedianEstimatedT80"]),
    ...
    handles = [
        ...
        for label, color in (("典型长寿命策略", colors["typical_long"]),
                             ("中间组", colors["middle"]),
                             ("典型短寿命策略", colors["typical_short"]))
    ]
    ax.legend(handles=handles, loc="lower center", bbox_to_anchor=(0.5, 1.02), ncol=3, frameon=False)
    ax.set_xlabel("平均充电时间（min）")
    ax.set_ylabel(r"预测循环寿命 $T_{80}$（cycle）")
    ax.set_title("充电时间与预测循环寿命的经验权衡")
    ax.grid(alpha=0.2)
    key_ax.axis("off")
    key_ax.set_title(r"点位编号（按预测 $T_{80}$ 排名）", loc="left", fontsize=10)
    ranked = frame.sort_values("PointT80Rank")
    mapping = [
        f"R{int(row.PointT80Rank)}  {POLICY_LABELS.get(row.Policy, row.Policy)}"
...
def _write_final_paper_report(path: Path, tables: dict[str, pd.DataFrame]) -> None:
    ...
    ranks = tables["strategy_lifetime_rank_stability"].sort_values("PointT80Rank")
    lifetimes = tables["strategy_lifetime_summary"].set_index("Policy")
    window = tables["lifetime_window_validation_summary"].loc[
        tables["lifetime_window_validation_summary"]["Selected"].astype(bool)
    ].iloc[0]
    residual = tables["residual_diagnostics_overall"].iloc[0]
    significant = tables["pairwise_strategy_lifetime_comparison"].loc[
        tables["pairwise_strategy_lifetime_comparison"]["SignificantAfterHolm"].astype(bool)
    ]
    selected = comparison.iloc[0]
    selection_pipeline = tables["selection_pipeline_summary"].iloc[0]
    lifetime_families = tables["lifetime_family_validation_summary"].sort_values(
        "StrategyEqualRMSE"
    )
    selected_lifetime_family = lifetime_families.loc[
        lifetime_families["SelectedFamily"].astype(bool)
    ].iloc[0]
    lifetime_battery_envelope = tables["lifetime_family_battery_envelope"]
    lifetime_strategy_envelope = tables["lifetime_family_strategy_envelope"]
    top = "、".join(ranks.head(3)["Policy"])
    bottom = "、".join(ranks.tail(3)["Policy"])
    lifetime_rows = []
    for rank_row in ranks.itertuples():
        row = lifetimes.loc[rank_row.Policy]
        lifetime_rows.append(
            f"| {int(rank_row.PointT80Rank)} | {rank_row.Policy} | {int(row.NBattery)} | "
            f"{row.MedianEstimatedT80:.1f} | [{row.BootstrapCI95Low:.1f}, {row.BootstrapCI95High:.1f}] |"
        )
    lines = [
        ...
        "## 模型选择结果",
        "",
        f"在外层留一电池、内层重新调参的SOH曲线候选家族比较中，函数型曲线RMSE为 {selected['MeanBatteryRMSE']:.6f}，样条基线为 {comparison.iloc[1]['MeanBatteryRMSE']:.6f}；完整选择流水线RMSE为 {selection_pipeline['MeanBatteryRMSE']:.6f}。寿命窗口回测中，40循环窗口的策略等权RMSE为 {window['StrategyEqualRMSE']:.6f}，平均电池RMSE为 {window['MeanBatteryRMSE']:.6f}，最坏电池RMSE为 {window['WorstBatteryRMSE']:.6f}。这两套验证分别回答SOH曲线重建和末段趋势预测，不能当作T80真值误差。",
        "",
        "## 寿命外推模型族敏感性",
        "",
        f"在局部线性、幂律和加速指数三种预注册单调外推族中，按L=150嵌套留一电池的151—200策略等权RMSE选择局部线性族，其RMSE为 {selected_lifetime_family['StrategyEqualRMSE']:.6f}；幂律和加速指数分别为 {lifetime_families.iloc[1]['StrategyEqualRMSE']:.6f}、{lifetime_families.iloc[2]['StrategyEqualRMSE']:.6f}。所选`linear_w40_s1`逐电池T80与主流水线完全一致。",
        "",
        f"三模型族给出的电池级T80最大/最小跨度比中位数为 {lifetime_battery_envelope['ModelFamilyT80Ratio'].median():.2f}、最大为 {lifetime_battery_envelope['ModelFamilyT80Ratio'].max():.2f}；策略级中位T80跨度比为 {lifetime_strategy_envelope['ModelFamilyMedianT80Ratio'].min():.2f}—{lifetime_strategy_envelope['ModelFamilyMedianT80Ratio'].max():.2f}。这些包络表达模型形式敏感性，不是置信区间。主bootstrap区间条件于已选局部线性族。",
        "",
        "## 策略比较",
        "",
        f"按每种策略的电池级预测T80中位数，前三位为：{top}；后三位为：{bottom}。排名稳定性应结合 `strategy_lifetime_rank_stability.csv` 中的Top/Bottom概率判断，不能只报告点排名。",
        "",
        "| 排名 | 快充策略 | 电池数 | T80中位数/cycle | 2000次整块电池bootstrap 95%区间 |",
        "|---:|---|---:|---:|---:|",
        *lifetime_rows,
        "",
        f"对任意两策略，以电池级预测T80均值差为统计量作双侧精确置换检验；36组策略对经Holm校正后显著的策略对为 {len(significant)} 组。每种策略只有3—8块电池，离散检验分辨率和外推模型不确定性都很有限；0组确证差异不表示策略寿命等价。",
        "",
        "## 解释与限制",
        "",
        f"SOH曲线模型训练残差平均一阶相关为 {residual['BatteryMeanLag1Correlation']:.3f}，说明平滑后仍存在循环内序列相关。T80把约0.8的终点外推到前150循环之外，个别电池的外推距离超过已观测前缀的70倍；bootstrap只传播已选局部线性族条件下的同策略电池差异，三族包络另行表达模型形式敏感性。因而T80适合比较与情景分析，不应写成已观测寿命或经过真实终点验证的绝对寿命。充电时间、温度和内阻关联仅有9个策略级样本，只作描述性机制线索，不作因果解释。",
        "",
        "## 论文取舍",
        "",
...
def _write_result_readme(path: Path) -> None:
    path.write_text(
        ...
        "当前附件没有观测到80% SOH终点，所有T80均为早期SOH趋势外推；"
        "151—200回测只验证近端趋势预测，不能替代真实寿命终点验证。\n",
        encoding="utf-8",
    )
